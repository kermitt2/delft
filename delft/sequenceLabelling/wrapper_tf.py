...
class Sequence(object):
        ...
        if self.model_config.transformer_name is None:
            ...
            for i in range(len(y_test)):
                if len(y_test[i]) != len(y_pred[i]):
                    ...
                    if len(y_test[i]) < len(y_pred[i]):
                        y_test[i] = y_test[i] + ["O"] * (
                            len(y_pred[i]) - len(y_test[i])
                        )
                    if len(y_test[i]) > len(y_pred[i]):
                        y_pred[i] = y_pred[i] + ["O"] * (
                            len(y_test[i]) - len(y_pred[i])
                        )
    ...
    def tag_file(
        self, file_in, output_format, file_out, batch_size=None, multi_gpu=False
    ):
        ...
        if self.model:
            ...
            with open(file_in, "r") as f:
                ...
                while (
                    texts is None
                    or len(texts) == self.model_config.batch_size * self.nb_workers
                ):
                    texts = next_n_lines(
                    ...
                    if first:
                        ...
                        if len(texts) < self.model_config.batch_size * self.nb_workers:
                            ...
                            jsonString = (
                                '{\n    "software": '
                                + json.dumps(
                                    annotations["software"], ensure_ascii=False
                                )
                                + ",\n"
                            )
                            jsonString += (
                                '    "date": '
                                + json.dumps(annotations["date"], ensure_ascii=False)
                                + ",\n"
                            )
                            jsonString += (
                                '    "model": '
                                + json.dumps(annotations["model"], ensure_ascii=False)
                                + ",\n"
                            )
                            jsonString += '    "texts": ['
                            if file_out == None:
                        ...
                        for jsonStr in annotations["texts"]:
                            jsonString = json.dumps(
                                jsonStr, sort_keys=False, indent=4, ensure_ascii=False
                            )
                            jsonString = re.sub("\n", "\n        ", jsonString)
                            if file_out == None:
    ...
    def load(
        ...
        self.p = Preprocessor.load(
            os.path.join(dir_path, self.model_config.model_name, PROCESSOR_FILE_NAME)
        )
        self.model = get_model(
            self.model_config,
            self.p,
        ...
        self.model.load(
            filepath=os.path.join(dir_path, self.model_config.model_name, weight_file)
        )
        self.model.print_summary()
