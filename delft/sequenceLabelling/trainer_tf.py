                ...
                if fold_id == 0:
                    ...
                    if self.model_config.transformer_name is not None:
                        ...
                        transformer_preprocessor.tokenizer.save_pretrained(
                            os.path.join(
                                output_directory, DEFAULT_TRANSFORMER_TOKENIZER_DIR
                            )
                        )
...
def get_callbacks(
    ...
    if log_dir:
        ...
        save_callback = ModelCheckpoint(
            os.path.join(log_dir, file_name), monitor="f1", save_weights_only=True
        )
...
class Scorer(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for i, (data, label) in enumerate(self.valid_batches):
            ...
            if isinstance(self.valid_batches, DataGeneratorTransformers):
                ...
                sequence_lengths = data[
                    -1
                ]  # this is the vectors "length_input" of the models input, always last
        ...
        if self.evaluation:
            ...
            self.report_as_map = (
                compute_metrics(y_true, y_pred) if has_data else compute_metrics([], [])
            )
...
def sparse_crossentropy_masked(y_true, y_pred):
    ...
    return tf.reduce_mean(
        tf.keras.losses.sparse_categorical_crossentropy(y_true_masked, y_pred_masked)
    )
