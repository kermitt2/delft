...
from delft.sequenceLabelling.data_generator import (
    DataGenerator,
...
class BaseModel(object):
    ...
    def init_transformer(
        ...
        transformer = Transformer(
            ...
            delft_local_path=local_path,
        )
        print(
            config.transformer_name,
            "will be used, loaded via",
            transformer.loading_method,
        )
        transformer_model = transformer.instantiate_layer(
            load_pretrained_weights=load_pretrained_weights
        )
...
class BidLSTM(BaseModel):
    ...
    def __init__(self, config, ntags=None):
        ...
        char_input = Input(
            shape=(None, config.max_char_length), dtype="int32", name="char_input"
        )
        ...
        self.model = Model(
            inputs=[word_input, char_input, length_input], outputs=[pred]
        )
        # self.model.summary()
        self.config = config
...
class BidLSTM_CNN(BaseModel):
    ...
    def __init__(self, config, ntags=None):
        ...
        self.model = Model(
            inputs=[word_input, char_input, casing_input, length_input], outputs=[pred]
        )
        self.config = config
...
class BidLSTM_CNN_CRF(BaseModel):
    ...
    def __init__(self, config, ntags=None):
        ...
        base_model = Model(
            inputs=[word_input, char_input, casing_input, length_input], outputs=[x]
        )
        self.model = CRFModelWrapperDefault(base_model, ntags)
        self.model.build(
            input_shape=[
...
class BidLSTM_CRF_FEATURES(BaseModel):
    ...
    def __init__(self, config, ntags=None):
        ...
        features_embedding = TimeDistributed(
            Embedding(
                input_dim=config.features_vocabulary_size * len(config.features_indices)
                + 1,
        ...
        base_model = Model(
            inputs=[word_input, char_input, features_input, length_input], outputs=[x]
        )
        self.model = CRFModelWrapperDefault(base_model, ntags)
        self.model.build(
            input_shape=[
...
class BERT(BaseModel):
    ...
    def __init__(
        ...
        transformer_layers = self.init_transformer(
            config, load_pretrained_weights, local_path, preprocessor
        )
        ...
        attention_mask = Input(
            shape=(None,), name="input_attention_mask", dtype="int32"
        )
...
class BERT_FEATURES(BaseModel):
    ...
    def __init__(
        ...
        self.model = Model(
            inputs=[input_ids_in, features_input, token_type_ids, attention_mask],
            outputs=[label_logits],
        )
        self.config = config
...
class BERT_CRF(BaseModel):
    ...
    def __init__(
        ...
        base_model = Model(
            inputs=[input_ids_in, token_type_ids, attention_mask], outputs=[x]
        )
...
class BERT_ChainCRF(BaseModel):
    ...
    def __init__(
        ...
        self.model = Model(
            inputs=[input_ids_in, token_type_ids, attention_mask], outputs=[pred]
        )
        self.config = config
