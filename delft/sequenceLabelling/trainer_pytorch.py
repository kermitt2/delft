...
class Trainer:
    ...
    def train(
        ...
        """
        # Compile model
        train_size = len(train_loader.dataset) if hasattr(train_loader, 'dataset') else len(train_loader) * self.training_config.batch_size
        self.compile_model(train_size)
        
        # Set up callbacks
        early_stopping = EarlyStopping(patience=self.training_config.patience)
        checkpoint = ModelCheckpoint(
            os.path.join(self.checkpoint_path, DEFAULT_WEIGHT_FILE_NAME) if self.checkpoint_path else DEFAULT_WEIGHT_FILE_NAME
        )
        ...
        # Load best model
        best_model_path = os.path.join(self.checkpoint_path, DEFAULT_WEIGHT_FILE_NAME) if self.checkpoint_path else DEFAULT_WEIGHT_FILE_NAME
    ...
    def evaluate(self, data_loader) -> Dict[str, float]:
        ...
        if self.preprocessor:
            ...
            # Parse report to get aggregate metrics
...
class Scorer:
    ...
    def on_epoch_end(
        ...
        if self.preprocessor:
            ...
            self.report = classification_report(true_labels, pred_labels, digits=4)
            ...
            for line in self.report.split('\n'):
                if 'micro avg' in line:
                    parts = line.split()
                    if len(parts) >= 4:
                        self.precision = float(parts[-4])
                        self.recall = float(parts[-3])
                        self.f1 = float(parts[-2])
                    break
