...
class BERT_CRF_FEATURES(BaseSequenceLabeler):
    ...
    def __init__(
        ...
        self.features_lstm = nn.LSTM(config.features_embedding_size, config.features_lstm_units, 
                                      batch_first=True, bidirectional=True)
        
        combined_size = hidden_size + config.features_lstm_units * 2
        
        self.bilstm = nn.LSTM(combined_size, config.num_word_lstm_units, 
                               batch_first=True, bidirectional=True)
        self.dropout = nn.Dropout(config.dropout)
        self.dense = nn.Linear(config.num_word_lstm_units * 2, config.num_word_lstm_units)
        self.crf = CRF(ntags)
    ...
    def forward(
        ...
        # Transformer
        transformer_out = self.transformer(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)
        text_emb = self.dropout(transformer_out.last_hidden_state)
