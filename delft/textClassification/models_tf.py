...
class BaseModel(object):
    ...
    def train_model(
        ...
        if validation_generator == None:
            ...
            while current_epoch <= max_epoch:
                ...
                if use_roc_auc:
                    print(
                        "Epoch {0} loss {1} best_loss {2} (for info) ".format(
                            current_epoch, total_loss, best_loss
                        )
                    )
                    print(
                        "Epoch {0} roc_auc {1} best_roc_auc {2} (for early stop) ".format(
                            current_epoch, total_roc_auc, best_roc_auc
                        )
                    )
                else:
                    print(
                        "Epoch {0} loss {1} best_loss {2} (for early stop) ".format(
                            current_epoch, total_loss, best_loss
                        )
                    )
                    print(
                        "Epoch {0} roc_auc {1} best_roc_auc {2} (for info) ".format(
                            current_epoch, total_roc_auc, best_roc_auc
                        )
                    )
    ...
    def predict(self, predict_generator, use_main_thread_only=False):
        ...
        y = self.model.predict(
            predict_generator, use_multiprocessing=multiprocessing, workers=nb_workers
        )
        return y
...
def train_folds(
    X, y, model_config, training_config, embeddings, models=None, callbacks=None
):
    ...
    for fold_id in range(0, fold_count):
        ...
        foldModel.train_model(
            model_config.list_classes,
        ...
        if model_config.transformer_name is None:
            ...
            directory = os.path.join(
                "data/models/textClassification/", model_config.model_name
            )
            ...
            model_path = os.path.join(
                directory, "model{0}_weights.hdf5".format(fold_id)
            )
...
def predict_folds(
    models, predict_generator, model_config, training_config, use_main_thread_only=False
):
    fold_count = model_config.fold_number
    y_predicts_list = []
    for fold_id in range(0, fold_count):
        if model_config.transformer_name is not None:
            ...
            model_path = os.path.join(
                "data/models/textClassification/",
        ...
        y_predicts = model.predict(
            predict_generator, use_main_thread_only=use_main_thread_only
        )
    ...
    y_predicts **= 1.0 / len(y_predicts_list)
...
class lstm(BaseModel):
    ...
    def __init__(self, model_config, training_config):
        ...
        # basic LSTM
        input_layer = Input(
            shape=(self.parameters["maxlen"], self.parameters["embed_size"]),
        )
        x = LSTM(
...
class cnn(BaseModel):
    ...
    def __init__(self, model_config, training_config):
        ...
        x = Conv1D(
            filters=self.parameters["recurrent_units"],
            kernel_size=2,
            padding="same",
            activation="relu",
        )(x)
...
class lstm_cnn(BaseModel):
    ...
    def __init__(self, model_config, training_config):
        ...
        x = Conv1D(
            filters=300, kernel_size=5, padding="valid", activation="tanh", strides=1
        )(x)
        x_a = GlobalMaxPool1D()(x)
        x_b = GlobalAveragePooling1D()(x)
        x = concatenate([x_a, x_b])
...
class dpcnn(BaseModel):
    ...
    def __init__(self, model_config, training_config):
        ...
        X = Conv1D(
            filters=self.parameters["recurrent_units"], kernel_size=2, strides=3
        )(X_shortcut1)
        X = Activation("relu")(X)
        X = Conv1D(
            filters=self.parameters["recurrent_units"], kernel_size=2, strides=3
