    ...
    def train(
        ...
        if self.training_config.early_stop:
            ...
            training_generator = DataGenerator(
                ...
                shuffle=True,
                bert_data=bert_data,
                transformer_tokenizer=self.model.transformer_tokenizer,
            )
    ...
    def train_nfold(
        ...
        if incremental:
            if self.models == None:
                ...
                return
            print(
                "Incremental n-fold training from loaded models",
    ...
    def eval(self, x_test, y_test, use_main_thread_only=False):
        ...
        if self.model_config.fold_number == 1:
            if self.model != None:
                ...
                result = self.model.predict(
                    test_generator, use_main_thread_only=use_main_thread_only
                )
            else:
                raise (OSError("Could not find a model."))
        else:
            ...
            test_generator = DataGenerator(
                ...
                shuffle=False,
                bert_data=bert_data,
                transformer_tokenizer=self.models[0].transformer_tokenizer,
            )
            result = predict_folds(
                self.models,
                test_generator,
        ...
        result_binary = np.array(
            [
                vectorize(xi, len(self.model_config.list_classes))
                for xi in result_intermediate
            ]
        )

        precision, recall, fscore, support = precision_recall_fscore_support(
            y_test, result_binary, average=None
        )
        print(
        ...
        for the_class in self.model_config.list_classes:
            ...
            print(
                "{:>14}  {:>12}  {:>12}  {:>12}  {:>12}".format(
                    the_class,
                    "{:10.4f}".format(precision[p]),
                    "{:10.4f}".format(recall[p]),
                    "{:10.4f}".format(fscore[p]),
                    support[p],
                )
            )
            p += 1
            ...
            for i in range(0, result.shape[0]):
                ...
                if len(np.unique(y_test[i, :])) == 1:
                    ...
                    roc_auc = roc_auc_score(
                        y_test[i, :], result[i, :], labels=[0.0, 1.0]
                    )
                total_roc_auc += roc_auc
    ...
    def save(self, dir_path="data/models/textClassification/"):
            ...
            if self.models == None:
                ...
                if self.model_config.transformer_name is None:
                    for i in range(0, self.model_config.fold_number):
                        self.models[i].save(
                            os.path.join(directory, "model{0}_weights.hdf5".format(i))
                        )
        ...
        if self.transformer_name is not None and self.model_config.fold_number == 1:
            ...
            if self.model.transformer_tokenizer is not None:
                self.model.transformer_tokenizer.save_pretrained(
        ...
        if self.model_config.fold_number == 1:
            ...
            if self.model_config.transformer_name is None:
                for i in range(0, self.model_config.fold_number):
                    ...
                    local_model.load(
                        os.path.join(model_path, "model{0}_weights.hdf5".format(i))
                    )
