...
```sh
# to the repository of its task, lfoppiano/grobid-model-header with the registry above
python -m delft.applications.hub_models push grobid-header-BidLSTM_CRF_FEATURES-potion-base-8M

# to the bucket of the registry
python -m delft.applications.hub_models push grobid-header-BidLSTM_CRF_FEATURES-potion-base-8M --bucket

# a model directory, to a given place, created private if it does not exist
python -m delft.applications.hub_models push /path/to/grobid-header-BERT_CRF --to hf://my-org/my-models --private
```
