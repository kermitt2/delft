#!/bin/bash
# Distributed training and evaluation script for all GROBID models with multiple architectures
# Uses sbatch for parallel job submission to fully exploit the cluster
#
# This script trains and evaluates all available models with the following architectures:
# - BidLSTM_CRF
...
LOG_DIR="${HOME}/slurm_logs/train_eval_distributed_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
submit_job() {
    ...
    local job_name="train_eval_${model}_${architecture}"
    ...
    if [[ "$model" == "header" ]] || [[ "$model" == "citation" ]]; then
        job_id=$(sbatch $SBATCH_OPTS \
            ...
            --output="$log_file" \
            --error="$log_file" \
            --wrap="$PYTHON_CMD $model train_eval --architecture $architecture --embedding $EMBEDDING --num-workers 6 --wandb" 2>&1 | grep -oP '\d+')
    else
        job_id=$(sbatch $SBATCH_OPTS \
            --job-name="$job_name" \
            --output="$log_file" \
            --error="$log_file" \
            --wrap="$PYTHON_CMD $model train_eval --architecture $architecture --embedding $EMBEDDING --wandb" 2>&1 | grep -oP '\d+')
    fi
...
submit_job_incremental() {
    ...
    if [[ "$model" == "header" ]] || [[ "$model" == "citation" ]]; then
        job_id=$(sbatch $SBATCH_OPTS \
            ...
            --output="$log_file" \
            --error="$log_file" \
            --wrap="$PYTHON_CMD $model train --architecture $architecture --embedding $EMBEDDING --incremental --num-workers 6 --wandb" 2>&1 | grep -oP '\d+')
    else
        job_id=$(sbatch $SBATCH_OPTS \
            --job-name="$job_name" \
            --output="$log_file" \
            --error="$log_file" \
            --wrap="$PYTHON_CMD $model train --architecture $architecture --embedding $EMBEDDING --incremental --wandb" 2>&1 | grep -oP '\d+')
    fi
...
echo "==========================================="
echo "Starting distributed training and evaluation of all GROBID models"
