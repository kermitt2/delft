...
submit_job() {
    ...
    if [[ "$model" == "header" ]] || [[ "$model" == "citation" ]]; then
        job_id=$(sbatch $SBATCH_OPTS \
            ...
            --output="$log_file" \
            --error="$log_file" \
            --wrap="$PYTHON_CMD $model train_eval --architecture $architecture --num-workers 6 --wandb" 2>&1 | grep -oP '\d+')
    else
        job_id=$(sbatch $SBATCH_OPTS \
            --job-name="$job_name" \
            --output="$log_file" \
...
submit_job_incremental() {
    ...
    if [[ "$model" == "header" ]] || [[ "$model" == "citation" ]]; then
        job_id=$(sbatch $SBATCH_OPTS \
            ...
            --output="$log_file" \
            --error="$log_file" \
            --wrap="$PYTHON_CMD $model train --architecture $architecture --incremental --num-workers 6 --wandb" 2>&1 | grep -oP '\d+')
    else
        job_id=$(sbatch $SBATCH_OPTS \
            --job-name="$job_name" \
            --output="$log_file" \
            --error="$log_file" \
            --wrap="$PYTHON_CMD $model train --architecture $architecture --incremental --wandb" 2>&1 | grep -oP '\d+')
    fi
