...
submit_job() {
    ...
    if [[ "$model" == "header" ]] || [[ "$model" == "citation" ]]; then
        job_id=$(sbatch $SBATCH_OPTS \
            ...
            --output="$log_file" \
            --error="$log_file" \
            --wrap="$PYTHON_CMD $model train --architecture $architecture --embedding $EMBEDDING --num-workers 6 --max-sequence-length 3000" 2>&1 | grep -oP '\d+')
    else
        job_id=$(sbatch $SBATCH_OPTS \
            --job-name="$job_name" \
            --output="$log_file" \
            --error="$log_file" \
            --wrap="$PYTHON_CMD $model train --architecture $architecture --embedding $EMBEDDING" 2>&1 | grep -oP '\d+')
    fi
